...
#define io_uring_for_each_cqe(ring, head, cqe)                                                                                                          \
    ...
    for (head = *(ring)->cq.khead;                                                                                                                      \
         (cqe = (head != io_uring_smp_load_acquire((ring)->cq.ktail) ? &(ring)->cq.cqes[io_uring_cqe_index(ring, head, (ring)->cq.ring_mask)] : NULL)); \
         head++)
...
void io_uring_prep_rw(int op, struct io_uring_sqe* sqe, int fd,
    ...
    __u64 offset)
{
    // Print whether the sqe is null or not
    printf("sqe is null: %d \r \n", sqe == NULL);
    // Print the address of the sqe
    printf("sqe address: %p \r \n", sqe);

    // Print whether the addr is null or not
    printf("addr is null: %d \r \n", addr == NULL);
    // Print the address of the addr
    printf("addr address: %p \r \n", addr);
...
void io_uring_prep_recvmsg_multishot(struct io_uring_sqe* sqe, int fd,
    struct msghdr* msg, unsigned flags)
{
...
{
    int header = msgh->msg_controllen + msgh->msg_namelen + sizeof(struct io_uring_recvmsg_out);
...
{
    printf("io_uring_get_sqe called \r \n");
    printf("ring address: %p \r \n", ring);

    printf("ring->sq.ktail = %d \r \n", *ring->sq.ktail);
    printf("ring->sq.khead = %d \r \n", *ring->sq.khead);
    printf("ring->sq.ring_mask = %d \r \n", ring->sq.ring_mask);
    printf("ring->sq.ring_entries = %d \r \n", ring->sq.ring_entries);
    printf("ring->sq.sqe_head = %d \r \n", ring->sq.sqe_head);
    printf("ring->sq.sqe_tail = %d \r \n", ring->sq.sqe_tail);
    ...
    if (next - head <= sq->ring_entries) {
        ...
        printf("returning new seq->sqes[%d] \r \n", (sq->sqe_tail & sq->ring_mask) << shift);
        return sqe;
    }

    printf("io_uring_get_sqe failed \r \n");
    return NULL;
}
...
int io_uring_example()
{
    ...
    printf("cqe->user_data:  %p (expected %p)", cqe->user_data, (void*)0xdeadbeef);
...
void* get_user_data(const struct my_struct* instance)
{
    return (void*)(unsigned long)instance->user_data;
}

void set_user_data(struct my_struct* instance, void* user_data)
{
    instance->user_data = (unsigned long)user_data;
}
